...
def controlHVAC():
  ...
  if hvac_power = 'on':
  	pass
    if last_temp_set < value:
  		client.publish("home/fan_power","on")
  	elif last_temp_set > value:
...
def automaticLights():
  while 1:
  	if last_motion_detected +timedelta(seconds=20) > datetime.datetime.now():
  	  difference = (last_motion_detected + timedelta(seconds=20) - datetime.datetime.now()).total_seconds()
